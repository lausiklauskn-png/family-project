...
## 2 · NEU und offen: die Schwelle 20 hat nicht gehalten

**Das ist die einzige echte Entscheidung, die diese Sitzung nicht selbst
getroffen hat.**
...
**33 Punkte Spanne an einer Seite, an der niemand etwas angefasst hat.** Der
Sprung 64 → 97 ist durch den Filter gelaufen und hat einen Journal-Eintrag
erzeugt, der wie ein Erfolg aussah. Das „Warum“ ist inzwischen ehrlich
nachgetragen: gar nichts.

`SCHWELLE = 20` in `tools/forschung.mjs` stammt aus einer gemessenen Streuung
von **19** — an derselben Seite, zwei Nächte vorher. Die Annahme ist widerlegt.

**Warum diese Sitzung sie NICHT hochgesetzt hat:** es gibt mehrere gleich
vernünftige Wege, und jeder kostet etwas.

| Weg | was er kostet |
|---|---|
| eigene, höhere Schwelle **nur für die Leistung** (die anderen drei stehen still: 92 · 100 · 100) | zwei Zahlen im Werkzeug statt einer |
| Schwelle **global** auf ~35 | die drei ruhigen Maße werden blind für echte Änderungen |
| Sprung erst melden, wenn ihn die **nächste** Messung bestätigt | der Eintrag kommt einen Tag später |
| nichts ändern, nur die Lehre | jede Nacht kann wieder ein Fehlalarm kommen |

Eine Zahl aus dem bisherigen Maximum abzuleiten ist ohnehin fragwürdig — die
19 war kein Naturgesetz, die 33 ist es genauso wenig (`LEHREN.md` Lehre 6c).
Deshalb: **Klaus fragen, dann bauen.** Und wenn gebaut wird, gilt die Regel
unverändert — **kein Wächter ohne Gegenprobe, jede einzelne, in beide
Richtungen.**
...
1. `forschung/LEHREN.md` — **Lehre 6c ist neu** (die 19 waren zu wenig), dazu
   1b (ein leerer Kasten verbirgt den Sprung darunter), 2 (erst den Trace),
   5 (Gegenprobe), 6 (eine Messung ist eine Stichprobe)
